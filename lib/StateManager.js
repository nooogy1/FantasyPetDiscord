...
class StateManager {
  constructor(stateFile = 'bot_state.json') {
    ...
    this.state = {
      ...
      queueTimings: {
        newPetQueues: {}, // channelId -> timestamp of last post
  ...
  /**
   * Record that a new pet queue was just processed for a channel
   */
  recordNewPetQueuePost(channelId) {
    this.state.queueTimings.newPetQueues[channelId] = new Date();
  }
  ...
  /**
   * Check if enough time has passed for next new pet queue post (15 minutes)
   */
  isTimeForNewPetQueuePost(channelId, intervalMinutes = 15) {
    const lastPost = this.state.queueTimings.newPetQueues[channelId];
  ...
  /**
   * Get time remaining until next new pet queue post
   */
  getTimeUntilNextNewPetPost(channelId, intervalMinutes = 15) {
    const lastPost = this.state.queueTimings.newPetQueues[channelId];
