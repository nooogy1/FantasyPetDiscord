...
class FilterHandler {
  ...
  async getPetCountWithFilters(filterData) {
    ...
    // Age group filter// Age group filter - no parameterization needed, just check the value
    if (filterData.ageGroup) {
      if (filterData.ageGroup === 'puppy') {
        query += ` AND (
          p.age ILIKE '%month%' OR p.age ILIKE '%week%' OR p.age ILIKE '%< 1%' OR p.age ILIKE '%less%' OR p.age ILIKE '%under 1%'
          OR (p.age ILIKE '%0 year%' AND NOT (p.age ILIKE '%1 year%' OR p.age ILIKE '%2 year%' OR p.age ILIKE '%3 year%'))
        )`;
      } else if (filterData.ageGroup === 'young') {
        query += ` AND (
          (p.age ILIKE '%1 year%' OR p.age ILIKE '%2 year%')
          AND NOT (p.age ILIKE '%3 year%' OR p.age ILIKE '%4 year%' OR p.age ILIKE '%5 year%' OR p.age ILIKE '%6 year%' OR p.age ILIKE '%7 year%')
        )`;
      } else if (filterData.ageGroup === 'adult') {
        query += ` AND (
          (p.age ILIKE '%3 year%' OR p.age ILIKE '%4 year%' OR p.age ILIKE '%5 year%' OR p.age ILIKE '%6 year%')
          AND NOT (p.age ILIKE '%7 year%' OR p.age ILIKE '%8 year%' OR p.age ILIKE '%9 year%' OR p.age ILIKE '%10 year%')
        )`;
      } else if (filterData.ageGroup === 'senior') {
        query += ` AND (
          p.age ILIKE '%7 year%' OR p.age ILIKE '%8 year%' OR p.age ILIKE '%9 year%' OR p.age ILIKE '%10 year%' OR p.age ILIKE '%11 year%' OR p.age ILIKE '%12 year%' OR p.age ILIKE '%13 year%'
        )`;
      }
    }
