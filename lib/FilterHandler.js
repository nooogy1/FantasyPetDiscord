...
class FilterHandler {
  ...
  async showAnimalTypeStep(message, userId) {
    ...
    const filter = (reaction, user) => {
      return user.id === userId && ['🐕', '🐈', '✨'].includes(reaction.emoji.name);
    };
    ...
    collector.on('collect', async (reaction) => {
      ...
      if (reaction.emoji.name === '🐕') {
        ...
        filter.animalType = null; // All animals
      }

      filter.currentStep = 'gender';
  ...
  async showGenderStep(message, userId) {
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields({ name: '\u200b', value: `Animal Type: **${filter.animalType || 'All'}**` })
      .setFooter({ text: 'React to select an option' })
      .setTimestamp();
    ...
    const reactionFilter = (reaction, user) => {
      return user.id === userId && ['♂️', '♀️', '🔵', '🟢', '✨'].includes(reaction.emoji.name);
    };
    ...
    collector.on('collect', async (reaction) => {
      ...
      if (reaction.emoji.name === '♂️') {
        ...
        filterData.gender = 'Spayed Female';
      } else if (reaction.emoji.name === '🟢') {
        filterData.gender = 'Neutered Male';
      } else if (reaction.emoji.name === '✨') {
        filterData.gender = null; // All genders
      }

      filterData.currentStep = 'age_group';
