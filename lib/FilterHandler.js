...
// UPDATED: Carousel is now ephemeral (private) - only user sees it
// UPDATED: Only broadcast to channel when pet is actually drafted
// UPDATED: Better emoji reactions with cleaner flow
...
class FilterHandler {
  ...
  async startFiltering(message, leagueId = null) {
    try {
      ...
      console.log(`🔍 Filter started by user ${userId}`);
      ...
      console.log(`✅ Filter state initialized for ${userId}`);
  ...
  async showAnimalTypeStep(message, userId) {
    ...
    const dogCount = await this.getPetCountWithFilters({ ...this.activeFilters.get(userId), animalType: 'Dog' });
    const catCount = await this.getPetCountWithFilters({ ...this.activeFilters.get(userId), animalType: 'Cat' });
    const allCount = await this.getPetCountWithFilters(this.activeFilters.get(userId));
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        { name: `🐕 Dogs`, value: `React with 🐕 (${dogCount} available)`, inline: false },
        { name: `🐈 Cats`, value: `React with 🐈 (${catCount} available)`, inline: false },
        { name: `✨ All Animals`, value: `React with ✨ (${allCount} available)`, inline: false }
      )
      .setFooter({ text: 'React to select an option' })
      .setTimestamp();

    // Send as EPHEMERAL (only user sees it)
    ...
    try {
      ...
      await message.reply({ 
        content: '❌ Error adding reactions. Check bot permissions.',
        ephemeral: true 
      });
      return;
    }
    ...
    const filter = (reaction, user) => {
      const isCorrectUser = user.id === userId;
      const isCorrectEmoji = ['🐕', '🐈', '✨'].includes(reaction.emoji.toString());
      return isCorrectUser && isCorrectEmoji;
    };

    const collector = reply.createReactionCollector({ filter, time: 300000, max: 1 });

    collector.on('collect', async (reaction) => {
      console.log(`🎯 Animal type selected: ${reaction.emoji.toString()}`);
      const filterData = this.activeFilters.get(userId);
      if (!filterData) {
        console.error('Filter data not found for user:', userId);
        return;
      }
      ...
      console.log('Selected animal type:', filterData.animalType);
    ...
    collector.on('end', (collected) => {
      if (collected.size === 0) {
        console.log('Animal type selection timed out');
        message.reply({ 
          content: '⏰ Filter selection timed out.',
          ephemeral: true 
        }).catch(() => {});
      }
    });
  ...
  async showGenderStep(message, userId) {
    ...
    // Get counts for each gender option
    const maleCount = await this.getPetCountWithFilters({ ...filterData, gender: 'Male' });
    const femaleCount = await this.getPetCountWithFilters({ ...filterData, gender: 'Female' });
    const allCount = await this.getPetCountWithFilters(filterData);
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        { name: `♂️ Male`, value: `React with ♂️ (${maleCount} available)`, inline: false },
        { name: `♀️ Female`, value: `React with ♀️ (${femaleCount} available)`, inline: false },
        { name: `✨ All Genders`, value: `React with ✨ (${allCount} available)`, inline: false }
      )
      .addFields(
        { name: '\u200b', value: `Animal Type: **${filterData.animalType || 'All'}**` }
      )
      .setFooter({ text: 'React to select an option' })
      .setTimestamp();
  ...
  async showAgeGroupStep(message, userId) {
    ...
    // Get counts for each age group option
    const puppyCount = await this.getPetCountWithFilters({ ...filterData, ageGroup: 'puppy' });
    const youngCount = await this.getPetCountWithFilters({ ...filterData, ageGroup: 'young' });
    const adultCount = await this.getPetCountWithFilters({ ...filterData, ageGroup: 'adult' });
    const seniorCount = await this.getPetCountWithFilters({ ...filterData, ageGroup: 'senior' });
    const allCount = await this.getPetCountWithFilters(filterData);
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        { name: `👶 Puppy/Kitten`, value: `React with 👶 (<1 year) - ${puppyCount} available`, inline: false },
        { name: `🧒 Young`, value: `React with 🧒 (1-3 years) - ${youngCount} available`, inline: false },
        { name: `👨 Adult`, value: `React with 👨 (3-7 years) - ${adultCount} available`, inline: false },
        { name: `👴 Senior`, value: `React with 👴 (7+ years) - ${seniorCount} available`, inline: false },
        { name: `✨ All Ages`, value: `React with ✨ - ${allCount} available`, inline: false }
      )
      .addFields(
  ...
  async showDaysInShelterStep(message, userId) {
    ...
    // Get counts for each days option
    const recentCount = await this.getPetCountWithFilters({ ...filterData, daysInShelter: { min: 0, max: 4 } });
    const moderateCount = await this.getPetCountWithFilters({ ...filterData, daysInShelter: { min: 4, max: 8 } });
    const longCount = await this.getPetCountWithFilters({ ...filterData, daysInShelter: { min: 8, max: 14 } });
    const veryLongCount = await this.getPetCountWithFilters({ ...filterData, daysInShelter: { min: 14, max: 999 } });
    const allCount = await this.getPetCountWithFilters(filterData);
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        { name: `🔴 Recent`, value: `React with 🔴 (0-4 days) - ${recentCount} available`, inline: false },
        { name: `🟠 Moderate`, value: `React with 🟠 (4-8 days) - ${moderateCount} available`, inline: false },
        { name: `🟡 Long`, value: `React with 🟡 (8-14 days) - ${longCount} available`, inline: false },
        { name: `🔵 Very Long`, value: `React with 🔵 (14+ days) - ${veryLongCount} available`, inline: false },
        { name: `✨ All`, value: `React with ✨ - ${allCount} available`, inline: false }
      )
      .addFields(
  ...
  async applyFiltersAndShowResults(message, userId) {
    try {
      ...
      if (pets.length === 0) {
        await message.reply({ 
          content: '❌ No pets match your filters. Try adjusting them!',
          ephemeral: true 
        });
  ...
  /**
   * Query database with applied filters and return COUNT only
   */
  async getPetCountWithFilters(filterData) {
    let query = `
    ...
    // Age group filter - IMPROVED with regex patterns
    if (filterData.ageGroup) {
      if (filterData.ageGroup === 'puppy') {
        query += ` AND (
          ...
          OR LOWER(p.age) ILIKE '%under 1%'
          OR LOWER(p.age) ILIKE '%newborn%'
          OR (LOWER(p.age) ~ '^0\\s*year' AND NOT (LOWER(p.age) ~ '[1-9]\\s*year'))
        )`;
      } else if (filterData.ageGroup === 'young') {
        query += ` AND (
          (LOWER(p.age) ~ '^[1-2]\\s*year' 
           OR LOWER(p.age) ILIKE '%1 year%' 
           OR LOWER(p.age) ILIKE '%2 year%'
           OR LOWER(p.age) ILIKE '%1-2%'
           OR LOWER(p.age) ILIKE '%1-3%')
          AND NOT (LOWER(p.age) ~ '[3-9]\\s*year' OR LOWER(p.age) ILIKE '%10 year%' OR LOWER(p.age) ILIKE '%3 year%' OR LOWER(p.age) ILIKE '%4 year%' OR LOWER(p.age) ILIKE '%5 year%' OR LOWER(p.age) ILIKE '%6 year%' OR LOWER(p.age) ILIKE '%7 year%')
        )`;
      } else if (filterData.ageGroup === 'adult') {
        query += ` AND (
          (LOWER(p.age) ~ '^[3-6]\\s*year' 
           OR LOWER(p.age) ILIKE '%3 year%' 
           OR LOWER(p.age) ILIKE '%4 year%' 
           OR LOWER(p.age) ILIKE '%5 year%' 
           OR LOWER(p.age) ILIKE '%6 year%'
           OR LOWER(p.age) ILIKE '%3-6%'
           OR LOWER(p.age) ILIKE '%3-7%')
          AND NOT (LOWER(p.age) ~ '[7-9]\\s*year' 
            OR LOWER(p.age) ~ '1[0-3]\\s*year'
            OR LOWER(p.age) ILIKE '%7 year%' 
            OR LOWER(p.age) ILIKE '%8 year%' 
            OR LOWER(p.age) ILIKE '%9 year%')
        )`;
      } else if (filterData.ageGroup === 'senior') {
        query += ` AND (
          LOWER(p.age) ~ '^[7-9]\\s*year' 
          OR LOWER(p.age) ~ '1[0-3]\\s*year'
          OR LOWER(p.age) ILIKE '%7 year%' 
          OR LOWER(p.age) ILIKE '%8 year%' 
          OR LOWER(p.age) ILIKE '%9 year%' 
          OR LOWER(p.age) ILIKE '%10 year%' 
          OR LOWER(p.age) ILIKE '%11 year%' 
          OR LOWER(p.age) ILIKE '%12 year%' 
          OR LOWER(p.age) ILIKE '%13 year%'
          OR LOWER(p.age) ILIKE '%7+%'
        )`;
      }
    }
  ...
  /**
   * Query database with applied filters and return full pet objects
   */
  ...
  /**
   * Show pet carousel - NOW EPHEMERAL (private)
   */
  async showPetCarousel(message, userId) {
    ...
    if (!pet) {
      await message.reply({ 
        content: '❌ No more pets to display.',
        ephemeral: true 
      });
      return;
    }
    ...
    // Send or update message - EPHEMERAL (private)
  ...
  setupCarouselCollector(message, userId, carouselMessage) {
    ...
    collector.on('collect', async (interaction) => {
      if (interaction.customId === `pet_prev_${userId}`) {
        ...
        await this.handleAdoptFlow(interaction, userId, petId, message);
      }
    });
   ...
   * UPDATED: Only broadcast to channel when pet is successfully drafted
   */
  async handleAdoptFlow(interaction, userId, petId, originalMessage) {
    try {
      const filterData = this.activeFilters.get(userId);
      ...
      if (pet.status !== 'available') {
        await interaction.reply({
          content: `❌ **[${pet.name}](${pet.pet_url})** is no longer available.`,
          ephemeral: true
        });
        return;
      }
      ...
      // BROADCAST TO CHANNEL (public) - Only on successful draft
      ...
      // Send to channel
      await interaction.channel.send({ embeds: [petCard] });
      ...
      if (filterData2.carouselMessage) {
        await this.showPetCarousel(originalMessage, userId);
      }
    } catch (error) {
