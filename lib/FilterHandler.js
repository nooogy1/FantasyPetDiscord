...
class FilterHandler {
  ...
  async showGenderStep(message, userId) {
    ...
    // Get count of pets matching current filters
    const petCount = await this.getPetCountWithFilters(filterData);
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        ...
        { name: '📊 Pets Available', value: `**${petCount}** pets match your filters so far`, inline: false }
      )
      .setFooter({ text: 'React to select an option' })
      .setTimestamp();
