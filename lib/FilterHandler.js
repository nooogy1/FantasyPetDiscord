...
// FIXED: Improved age group filtering logic
// UPDATED: All pet references now hyperlinked with pet_url
...
class FilterHandler {
  ...
  async showAnimalTypeStep(message, userId) {
    ...
    try {
      ...
      await message.reply('❌ Error adding reactions. Check bot permissions.');
      return;
    }
  ...
  async showPetCarousel(message, userId) {
    ...
    const embed = new EmbedBuilder()
      .setColor('#3498db')
      .setTitle(`[${pet.name}](${pet.pet_url}) ([${pet.pet_id}](${pet.pet_url}))`)
  ...
  async handleAdoptFlow(interaction, userId, petId) {
    try {
      ...
      // Send confirmation to user (public)
      await interaction.reply({
        embeds: [embed]
      });

      // Also broadcast to channel as a public message
