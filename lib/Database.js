// lib/Database.js - PostgreSQL Database Interface with Simplified Queue Support
...
class Database {
  ...
  async getAllPets() {
    const query = `
      SELECT 
        ...
        discord_available_posted, discord_adopted_posted, discord_photo_url
      FROM pets
      ORDER BY pet_id
    `;
  ...
  /**
   * Add eligible COMPLETE pets to queue for a specific channel
   * Only queues pets that have BOTH name and real photo
   */
  async queuePetsForChannel(channelId, leagueId) {
  ...
  /**
   * Get next pet to post from queue for a specific channel
   */
  async getNextPetToPost(channelId) {
    const query = `
      SELECT 
        dqi.id as queue_id,
        dqi.pet_id,
