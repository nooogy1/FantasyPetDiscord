...
class QueueManager {
  ...
  /**
   * Process new pet queue for all channels (every 15 minutes per channel)
   */
  async processNewPetQueues(channelConfigs) {
    console.log('🎯 Processing new pet queues...');
    ...
    for (const config of channelConfigs) {
      ...
      const petToPost = await this.db.getNextNewPetToPost(channelId);
      ...
      try {
        ...
        const embed = this.createNewPetEmbed(petToPost, leagueName);
        await channel.send({ embeds: [embed] });

        console.log(`   ✅ Posted new pet: ${petToPost.name} (${petToPost.pet_code}) to ${leagueName}`);
        ...
        console.error(`   ❌ Error posting new pet to channel ${channelId}:`, error.message);
      }
    }
  }
  ...
  createNewPetEmbed(pet, leagueName) {
    ...
    const embed = new EmbedBuilder()
      ...
      .setFooter({ text: 'React or use !addpet to draft this pet' })
      .setTimestamp();
